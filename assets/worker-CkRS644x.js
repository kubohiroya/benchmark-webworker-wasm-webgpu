...
}`;let d,T,U,L,W,Z,R,I,y,P,v;async function me(e,n){if(navigator.gpu===void 0){console.error("WebGPU is not supported");return}const t=await navigator.gpu.requestAdapter();if(!t)throw new Error("Failed to get GPU adapter");d=await t.requestDevice();const c=document.createElement("canvas");document.body.appendChild(c),c.width=e,c.height=n,P=e,v=n,T=c.getContext("webgpu");const o=navigator.gpu.getPreferredCanvasFormat();T.configure({device:d,format:o,alphaMode:"opaque"});const i=d.createShaderModule({code:de});U=d.createComputePipeline({layout:"auto",compute:{module:i,entryPoint:"main"}}),L=new ArrayBuffer(7*4),W=d.createBuffer({size:L.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),I=e*n*4,R=d.createBuffer({size:I,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC}),y=d.createBuffer({size:I,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),Z=d.createBindGroup({label:"bindGroup",layout:U.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:W}},{binding:1,resource:{buffer:R}}]})}async function we(e,n,t,c,o){const i=new Uint32Array(L),f=new Float32Array(L);i.set([P,v,e],0),f.set([n,t,c,o],3),d.queue.writeBuffer(W,0,L);const A=d.createCommandEncoder(),s=A.beginComputePass();s.setPipeline(U),s.setBindGroup(0,Z),s.dispatchWorkgroups(Math.ceil(P/8),Math.ceil(v/8),1),s.end(),A.copyBufferToBuffer(R,0,y,0,I);const r=A.finish();d.queue.submit([r]),await d.queue.onSubmittedWorkDone(),await y.mapAsync(GPUMapMode.READ);const a=new Uint8ClampedArray(y.getMappedRange().slice(0,I));return y.unmap(),a}const Ce={async generateMandelbrotSingle(e,n,t,c,o){const{width:i,height:f,maxIterations:A,minX:s,maxX:r,minY:a,maxY:u}=t;switch(this.hasCanceled=!1,n){case g.JS_SINGLE:{Y(i,f),re(A,s,r,a,u),c(e,E);break}case g.WASM_SINGLE:{z(i,f);const l=ne(A,s,r,a,u),B=new Uint8ClampedArray(F.buffer,l,i*f*4);c(e,B);break}case g.WEBGPU_SINGLE:{await me(i,f);const l=await we(A,s,r,a,u);c(e,l);break}default:throw new Error(`unknown runtime: ${n}`)}o()},async generateMandelbrotLineByLine(e,n,t,c,o,i){const{width:f,height:A,maxIterations:s,minX:r,maxX:a,minY:u,maxY:l}=t;switch(this.hasCanceled=!1,n){case g.JS_MULTI:case g.JS_MULTI_CANCELLABLE:case g.JS_WORKER:Y(f,A);break;case g.WASM_MULTI:case g.WASM_MULTI_CANCELLABLE:case g.WASM_WORKER:z(f,A);break;default:throw new Error(`unknown runtime: ${n}`)}const B=f*4;for(let m=0;m<A;m++){switch(n){case g.JS_MULTI_CANCELLABLE:case g.JS_WORKER:case g.WASM_MULTI_CANCELLABLE:case g.WASM_WORKER:if(this.hasCanceled){i();return}}switch(n){case g.JS_MULTI:case g.JS_MULTI_CANCELLABLE:case g.JS_WORKER:{N(m,s,r,a,u,l);const w=E.slice(m*B,m*B+B);n===g.JS_WORKER?c(e,m,V(w,[w.buffer])):c(e,m,w);break}case g.WASM_MULTI:case g.WASM_MULTI_CANCELLABLE:case g.WASM_WORKER:{const w=te(m,s,r,a,u,l),$=new Uint8ClampedArray(F.buffer,w,B);c(e,m,$);break}default:throw new Error(`unknown runtime: ${n}`)}switch(n){case g.JS_MULTI_CANCELLABLE:case g.WASM_MULTI_CANCELLABLE:await new Promise(w=>setTimeout(w,0))}}o()},hasCanceled:!1,cancel(){console.log("cancel"),this.hasCanceled=!0},[S](){console.log("finalizer"),this.cancel()}};k(Ce);var be=Object.freeze({__proto__:null});
